...
const PostProcessing = ({ strength = 1, threshold = 1, fullScreenEffect = true, }) => {
    ...
    const render = useMemo(() => {
        ...
        const withScanEffect = mix(scenePassColor, add(scenePassColor, redOverlay), fullScreenEffect ? smoothstep(0.9, 1.0, oneMinus(scanLine)) : 1.0);
...
const Scene = () => {
    ...
    return (<mesh ref={meshRef} scale={[w * scaleFactor, h * scaleFactor, 1]} material={material}>
      <planeGeometry />
    </mesh>);
};
export const HeroFuturistic = () => {
    ...
    return (<div className="h-svh relative bg-black overflow-hidden">
      <div className="h-svh uppercase items-center w-full absolute z-50 pointer-events-none px-10 flex justify-center flex-col">
        <div className="text-3xl md:text-5xl xl:text-6xl 2xl:text-7xl font-extrabold">
          <div className="flex space-x-2 lg:space-x-6 overflow-hidden text-white">
            {titleWords.map((word, index) => (<div key={index} className={`transition-opacity duration-500 ${index < visibleWords ? 'opacity-100' : 'opacity-0'}`} style={{
        ...
        <div className="text-xs md:text-xl xl:text-2xl 2xl:text-3xl mt-2 overflow-hidden text-white font-bold">
          <div className={`transition-opacity duration-700 ${subtitleVisible ? 'opacity-100' : 'opacity-0'}`} style={{
      ...
      <div className="absolute bottom-20 left-0 right-0 flex justify-center z-50 pointer-events-auto">
        <Link href="/builder">
          <button className="group relative px-8 py-4 bg-transparent border border-white/20 text-white font-bold rounded-full hover:bg-white/10 transition-all duration-300 flex items-center gap-3 backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4 fill-mode-forwards opacity-0" style={{ animationDelay: '2.2s', animationFillMode: 'forwards' }}>
            Start Building
